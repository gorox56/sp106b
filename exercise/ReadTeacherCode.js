...
function assemble(asmFile, objFile) {
    ...
    c.log(JSON.stringify(lines, null, 2));   //(obj, fn, indent) 
    pass1(lines);
    pass2(lines, objFile);
}

function parse(line, i) {
    line.match(/^([^\/]*)(\/.*)?$/);         ///後為註解，故只取/前
    line = RegExp.$1.trim();                 //消除兩邊空白
    if (line.length===0)
      return null;                   
    if (line.startsWith("@")) {
    ...
    } else if (line.match(/^((([AMD]*)=)?([AMD01\+\-\&\|\!]*))(;(\w*))?$/)) { // \w=[a-zA-Z0-9_]
...
function pass1(lines) {
    ...
    for (var i=0; i<lines.length; i++) {
      var p = parse(lines[i], i);             //剖析
      if (p===null) continue;                 //跳過註解或空行
      if (p.type === "S") {
...
function pass2(lines, objFile) {
    ...
    ws.once('open', function(fd) {
      ...
      for (var i=0; i<lines.length; i++) {
        ...
        if (p===null || p.type === "S") continue;       //空行或符號就跳過
        var code = toCode(p);
...
function toCode(p) {
    var address; 
    if (p.type === "A") {
      if (p.arg.match(/^\d+$/)) {    //\d=[0-9]
        address = parseInt(p.arg);              //將字串轉換為數字
      } else {
        address = symTable[p.arg]; 
        if (typeof address === 'undefined') {
          address = symTop;
          addSymbol(p.arg, address);            //初登場的就加入符號表      
        }
    }
    return address; 
  } else { // if (p.type === "C")
